...
@section('title','Edit Category | POS')
@section('content')
@section('menu')
<ul class="app-menu">
            ...
            <div class="tile-body">
              <form role="form" action="{{url('/update-category/'.$category->id)}}" method="post">
                ...
                <div class="form-group{{$errors->has('category_name') ? 'has-error' : '' }}">
                  ...
                  <input class="form-control" name="category_name" id="category_name" type="text" placeholder="Enter Category Name" value="{{$category->category_name}}">
                  @if($errors->has('category_name'))
                    <span class="text-danger">
                        <small>{{$errors->first('category_name')}}</small>
                    </span>
				  @endif
                </div>
